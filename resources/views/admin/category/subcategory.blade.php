                        ...
                        <tbody class="list">
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Action')">
                                    <a href="javascript:void(0)" class="icon-btn btn--primary editBtn" data-name="{{ $subcategory->name }}" data-catid="{{ $subcategory->category_id }}" data-action="{{ route('admin.subcategory.update',$subcategory->id) }}"><i class="la la-pen"></i></a>
                                    @if($subcategory->status == 1)
                                    <a href="{{ route('admin.subcategory.status',$subcategory->id) }}" class="icon-btn btn--danger"><i class="la la-eye"></i></a>
                                    @else
                                    <a href="{{ route('admin.subcategory.status',$subcategory->id) }}" class="icon-btn btn--success"><i class="la la-eye-slash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            @endif
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">@lang('Add Sub Category')</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn--dark" data-dismiss="modal">@lang('Close')</button>
        <button type="submit" class="btn btn--primary">@lang('Add Sub Category')</button>
      </div>
  </form>
    </div>
  </div>
...
<script>
    (function($){
        ...
        $('.editBtn').on('click',function(){
            ...
            modal.find('select[name=category_id]').val($(this).data('catid'));
