...
function videoPath($time){
    return $time->format('Y').'/'.$time->format('M').'/'.$time->format('d');
}

function upload_video($file,$location, $old = null){
    $path = makeDirectory($location);
    if (!empty($old)) {
        removeFile('assets/videos/'.$old);
    }
    $video = uniqid().time().'.'.$file->getClientOriginalExtension();
    move_uploaded_file($file->getPathName(), $location.'/'.$video);
    return $video;
}


function upload_disk_video($file,$location,$fileExtension,$disk){
    diskConfigure();
    $disk = \Illuminate\Support\Facades\Storage::disk($disk);
    $path = ftpMakeDirectory($location,$disk);
...
function ftpMakeDirectory($path,$disk){
    if ($disk->exists($path)) {
        return true;
    }
...
function removeVideoFile($video,$disk){
    diskConfigure();
...
function diskConfigure(){
    ...
    Config::set('filesystems.disks.s3.key',$gnl->aws->access_key);
    Config::set('filesystems.disks.s3.secret',$gnl->aws->secret_access_key);
    Config::set('filesystems.disks.s3.region',$gnl->aws->regeion);
...
function showAd(){
    ...
    if ($ad) {
        if($ad->type == 'banner'){
            $final = '<a href="'.$ad->content->link.'" class="ad advertise" data-id="'.$ad->id.'" target="_balnk"><img src="'.getImage('assets/images/ads/'.$ad->content->image).'"></a>';
            $id = $ad->id;
        }else{
